...
export function AuthScreen({ mode }: { mode: "signin" | "signup" }) {
  ...
  return <main className="auth-page">
    <section className="auth-story">
      <div className="auth-brand"><i><ShieldCheck size={24} /></i><div><b>SOVEREIGN<span>FORGE</span></b><small>LOCAL AI CONTROL PLANE</small></div></div>
      <div><span className="eyebrow">SECURE LOCAL ACCESS</span><h1>Your confidential intelligence stays inside your boundary.</h1><p>Authenticate against your organization&apos;s local SovereignForge instance. Credentials, sessions, documents, and model workloads remain under your control.</p></div>
    ...
    <section className="auth-panel">
      <div className="auth-form-wrap">
        ...
        <h2>{signup ? "Create your organization" : "Sign in to SovereignForge"}</h2>
        ...
        <p className="auth-switch">{signup ? "Already have an account?" : "New to SovereignForge?"} <Link href={signup ? "/signin" : "/signup"}>{signup ? "Sign in" : "Create organization"}</Link></p>
      </div>
    </section>
  </main>;
}
